...
(() => {
  ...
  document.addEventListener("app-ready", () => {
    ...
    const balanzaValorDivisor = document.getElementById("balanza-valor-divisor");
    const balanzaCodigoInicio = document.getElementById("balanza-codigo-inicio");
    const balanzaCodigoLongitud = document.getElementById("balanza-codigo-longitud");
    const balanzaValorInicio = document.getElementById("balanza-valor-inicio");
    const balanzaValorLongitud = document.getElementById("balanza-valor-longitud");
    ...
    const descuentoEfectivoInput = document.getElementById("descuento-efectivo");
    ...
    const horariosArqueoContainer = document.getElementById("horarios-arqueo-container");
    const arqueoConfigForm = document.getElementById("arqueo-config-form");
    const horarioAperturaT1Input = document.getElementById("horario-apertura-t1");
    const horarioCierreT1Input = document.getElementById("horario-cierre-t1");
    const horarioAperturaT2Input = document.getElementById("horario-apertura-t2");
    ...
    const buildBarcodeFromConfig = (cfg, { plu, priceCent = 0, weightGr = 0 }) => {
    ...
    const refreshBarcodePreview = () => {
      ...
      if (barcodeAutoChk?.checked) {
        ...
        barcodePreview.textContent = `Código manual: ${pluBarcodeInput?.value || ""}`;
      }
    };
    ...
    const loadAvailablePorts = async () => {
      ...
      try {
        const { serialPorts = [], printers = [] } = await ipcInvoke("get-available-ports");
        const currentScanner = scannerPortSelect.value;
        const currentPrinter = printerNameSelect.value;

        scannerPortSelect.innerHTML = '<option value="">-- No seleccionado --</option>';
        printerNameSelect.innerHTML = '<option value="">-- No seleccionada --</option>';
        ...
        (empleados || []).forEach((emp) => {
          ...
          card.innerHTML = `
            ...
            <div class="card-actions">
              <button class="btn btn-info btn-sm btn-edit-empleado" data-id="${emp.id}">Editar</button>
              <button class="btn btn-danger btn-sm btn-delete-empleado" data-id="${emp.id}">Eliminar</button>
            </div>`;
        ...
        (gastos || []).forEach((gasto) => {
          ...
          item.innerHTML = `
            ...
            <div class="d-flex align-items-center" style="gap: 5px;">
              ...
              <button class="btn btn-danger btn-sm btn-delete-gasto" data-id="${gasto.id}">🗑️</button>
            </div>`;
    ...
    const loadAdminConfig = async () => {
      try {
        ...
        if (afipForm) {
          ...
          certFilePathDisplay.textContent = config.afip_cert_path || "No seleccionado";
          keyFilePathDisplay.textContent = config.afip_key_path || "No seleccionado";
        }
        ...
        if (businessInfoForm) {
          ...
          if (config.logo_url) {
            const logoSrc = `app:///${String(config.logo_url).replace(/\\/g, "/")}?${Date.now()}`;
        ...
        if (scaleConfigForm) {
          ...
          if (scaleProtocol) scaleProtocol.value = sc.protocol || "kretz-report";
          if (scaleTimeout) scaleTimeout.value = sc.timeoutMs ?? 4000;

          const toggleTransp = () => {
            document.querySelectorAll(".only-tcp").forEach(el => el.style.display = (scaleTransport?.value === "tcp") ? "" : "none");
            document.querySelectorAll(".only-bt").forEach(el => el.style.display = (scaleTransport?.value === "bt") ? "" : "none");
          };
          toggleTransp();
    ...
    on(userForm, "submit", async (e) => {
      ...
      try {
        ...
        if (result?.success) {
          ...
          toast.show(result?.message || "No se pudo guardar el usuario.", "error");
        }
      } catch (err) {
        console.error(err);
    ...
    on(usersTableBody, "click", async (e) => {
      ...
      if (btn.classList.contains("btn-edit")) {
        ...
        user ? openModal(user) : toast.show("No se encontró el usuario.", "error");
      }
    ...
    on(btnVerifyMP, "click", async () => {
      ...
      try {
        ...
        if (result?.success) {
          posIdInput.innerHTML = '<option value="">-- Selecciona una caja --</option>';
          ...
          toast.show(
            (result.data || []).length ? "Cajas cargadas." : "No se encontraron cajas.",
            (result.data || []).length ? "success" : "info"
          );
        } else {
          toast.show(`Error: ${result?.message || "Verificación fallida"}`, "error");
          posIdInput.innerHTML = '<option value="">-- Verificación fallida --</option>';
        }
      } catch {
    ...
    on(empleadosContainer, "click", async (e) => {
      ...
      if (editBtn) {
        ...
        const empleados = await ipcInvoke("get-empleados");
        const empleado = (empleados || []).find((emp) => String(emp.id) === String(id));
      ...
      on(afipForm, "submit", async (e) => {
        ...
        try {
          ...
          result?.success
            ...
            : toast.show("Error: " + (result?.message || "No se pudo guardar."), "error");
        } catch {
    ...
    on(syncConfigForm, "submit", async (e) => {
      ...
      try {
        ...
        if (!saved?.success) {
          toast.show(saved?.message || "No se pudo guardar la sincronización.", "error");
          return;
        }
        ...
        if (result?.success) {
          ...
          toast.show(result?.message || "Token inválido o error de conexión.", "error");
        }
      } catch (e1) {
        console.error(e1);
