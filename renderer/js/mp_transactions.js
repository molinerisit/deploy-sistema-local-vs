...
document.addEventListener('app-ready', () => { // Usamos app-ready por consistencia
    ...
    const loadTransactions = async () => {
        ...
        try {
            ...
            tableBody.innerHTML = `<tr><td colspan="4" style="text-align:center;color:red;">${errorMessage}</td></tr>`;
    ...
    const renderTable = (transactions) => {
        if (!transactions || transactions.length === 0) {
            tableBody.innerHTML = '<tr><td colspan="4" style="text-align:center;">No se encontraron transacciones.</td></tr>';
            return;
        }
